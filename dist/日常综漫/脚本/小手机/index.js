(function () {
  ...
  var STYLE_ID = "sp-little-phone-style";
    ...
    // Splash screen
    '.sp-chat{display:flex;flex-direction:column;height:100%}',
    ...
    '.sp-chat{display:flex;flex-direction:column;height:100%;background:#ededed}',
    ...
    '.sp-chat-msgs{flex:1;overflow-y:auto;padding:8px 10px;display:flex;flex-direction:column;gap:2px}',
    '.sp-chat-msgs::-webkit-scrollbar{display:none}',
    '.sp-chat-msg-row{display:flex;gap:8px;margin-bottom:4px;align-items:flex-start}',
    '.sp-chat-msg-row--self{flex-direction:row-reverse}',
    '.sp-chat-avatar{width:32px;height:32px;border-radius:4px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff}',
    '.sp-chat-bubble{max-width:70%;padding:8px 10px;border-radius:4px;font-size:12px;line-height:1.5;word-break:break-word;position:relative}',
    '.sp-chat-bubble--self{background:#95ec69;color:#000}',
    '.sp-chat-bubble--other{background:#fff;color:#000}',
    ...
    '.sp-chat-sysmsg{text-align:center;font-size:10px;color:#999;margin:4px 0}',
    ...
    '.sp-chat-npc{flex-shrink:0;display:flex;flex-direction:column;align-items:center;gap:2px;cursor:pointer;opacity:.4;transition:opacity .2s}',
    '.sp-chat-npc.active{opacity:1}',
    '.sp-chat-npc-avatar{width:28px;height:28px;border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#fff}',
  ...
  var APPS = [
    { id: 'chat', icon: '💬', label: '聊天', tint: 'linear-gradient(135deg,#07c160,#06ad56)' },
  ...
  function renderAppScreen() {
    ...
    switch (activeApp) {
      case 'chat':
        html += '<div class="sp-chat">';
        html += '<div class="sp-chat-header">';
        html += '<button class="sp-chat-header-back sp-back-btn"><i class="fa-solid fa-chevron-left"></i></button>';
        html += '<span class="sp-chat-header-title">群聊</span>';
        html += '</div>';
        ...
        if (settings.chatMessages.length === 0) {
          html += '<div class="sp-chat-empty">点击上方角色头像添加参与者，然后开始聊天</div>';
        }
        html += '</div>';
  ...
  function init() {
    ...
    timer = setInterval(updateTime, 30000);    eventOn(tavern_events.GENERATION_ENDED, function () {
